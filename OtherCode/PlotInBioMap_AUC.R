...
s_REGm <- read.delim("OtherNetworks/Diff_str/2--InBioMap_re/m/Re_Sum_C.txt")
s_REGml <- read.delim("OtherNetworks/Diff_str/2--InBioMap_re/ml/Re_Sum_C.txt")
s_REGmlw <- read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlw/Re_Sum_C.txt")
s_REGmlwp <- read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwp/Re_Sum_C.txt")
s_REGmlwpFC <- ead.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwpFC/Re_Sum_C.txt")
...
#DEGm
NormalDPRim<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/m/NormalDPR.txt") 
dataD_im<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/m/dataD.txt")

Re_DEGm<-REfunction(NormalDPRim,dataD_im,s_REGm,"REG","binormal")
gg_REGm1<- Re_DEGm[[1]]
zz_REGm<- Re_DEGm[[2]]
...
#DEGml
NormalDPRil<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/ml/NormalDPR.txt") 
dataD_il<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/ml/dataD.txt")
...
#DEGmlw
NormalDPRimlw<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlw/NormalDPR.txt") 
dataD_imlw<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlw/dataD.txt")
...
#DEGmlwp
NormalDPRimlwp<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwp/NormalDPR.txt") 
dataD_imlwp<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwp/dataD.txt")
...
#DEGmlwp
NormalDPRimlwp<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwpFC/NormalDPR.txt") 
dataD_imlwp<-read.delim("OtherNetworks/Diff_str/2--InBioMap_re/mlwpFC/dataD.txt")
...
IBa1 <- ggplot(data=ggtest,aes(x=x1,y=y1,group = type))+geom_line(aes(x=x1,y=y1,color = type),size = 1,linetype = 1)+labs(y="Sensitivity",x="1-Specificity",
...
IBa1<-ggplot(data=ggtest,aes(x=x1,y=y1,group = type))+geom_line(aes(x=x1,y=y1,color = type),size = 1,linetype = 1)+
  labs(y="Sensitivity",x="1-Specificity",title="InBio_Map" ,family = "Times", color="black")+
  theme_bw()+
  theme(legend.justification=c(1,0), legend.position=c(0.98,0.015),legend.key = element_blank())+
  theme(panel.border = element_blank(),panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  scale_color_manual(values=c('steelblue','hotpink2',"#99cc00","gold1") ,"    Parameter               AUC",
                     labels=c(paste("REGml", as.character(round(REGm_info_P[dim(REGm_info_P)[1]-1,dim(REGm_info_P)[2]],3)),sep="              "),
                              paste("REGmlw", as.character(round(REGml_info_P[dim(REGml_info_P)[1]-1,dim(REGml_info_P)[2]],3)),sep="           "),
                              paste("REGmlwp", as.character(round(REGmlw_info_P[dim(REGmlw_info_P)[1]-1,dim(REGmlw_info_P)[2]],3)),sep="        "),
                              paste("REGmlwpFC", as.character(round(REGmlwp_info_P[dim(REGmlwp_info_P)[1]-1,dim(REGmlwp_info_P)[2]],3)),sep="    ")))+
  xlim(0,1)+ylim(0,1)+ 
...
IBa1
# pdf("1--DifferentStrategies/Plots/HumanNet_StrategiesAUC.pdf")
# IBa1
# dev.off()
# png("1--DifferentStrategies/Plots/HumanNet_StrategiesAUC.png")
# IBa1
# dev.off()
